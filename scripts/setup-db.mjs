// Setup database tables via Supabase service role
// Uses the SQL endpoint available through supabase-js

const SUPABASE_URL = "https://yuauqnfqdcjbnuczpgxf.supabase.co";
const SERVICE_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inl1YXVxbmZxZGNqYm51Y3pwZ3hmIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MjI5Mjg2MywiZXhwIjoyMDg3ODY4ODYzfQ.nm3shIj3HXTmdlDAXqw7uBlbIAMKOByxg65UhtorchU";
...
async function runSQL(sql, label) {
  ...
  if (!res.ok) {
    ...
    // If exec_sql doesn't exist, we'll create it
    if (text.includes("Could not find the function")) {
      return { error: "no_exec_sql" };
    }
...
// First, try to create exec_sql function via the /pg endpoint (Supabase Edge)
...
async function createExecSql() {
  ...
  // Try through rpc first with a raw query approach
  const res = await fetch(`${SUPABASE_URL}/rest/v1/rpc/exec_sql`, {
    method: "POST",
    headers: {
      apikey: SERVICE_KEY,
...
// Alternative: Run individual CREATE TABLE statements as separate RPC calls
// is not possible through PostgREST alone.
// We'll write the SQL for the user to paste into the Supabase SQL Editor.
...
// Since we can't run SQL directly via REST, output the SQL file
// and try the Supabase management API approach
...
const outPath = join(__dirname, "schema.sql");
writeFileSync(outPath, sql);
console.log(`SQL written to: ${outPath}`);
console.log(`\nTotal length: ${sql.length} chars`);
console.log("\nAttempting to run via Supabase...");

// Try using the management API
async function runViaManagementAPI() {
  // The Supabase management API for running SQL is at:
  // POST https://api.supabase.com/v1/projects/{ref}/database/query
  // But this requires a management API token (different from service role)
  //
  // Alternative: Use the pg endpoint that newer Supabase projects expose
  // POST https://{ref}.supabase.co/pg/query

  // Try the newer /functions/v1 approach or direct pg wire
  // Actually, let's just try connecting directly with the pooler

  // From the project ref, the pooler connection would be:
  const poolerHost = "aws-0-eu-central-1.pooler.supabase.com";

  // Actually let's just use the supabase-js client to verify connectivity
  // and then guide the user to paste SQL in the dashboard

  const res = await fetch(`${SUPABASE_URL}/rest/v1/style_templates?select=count`, {
    headers: {
      apikey: SERVICE_KEY,
      Authorization: `Bearer ${SERVICE_KEY}`,
    },
  ...
  if (res.status === 404 || res.status === 400) {
    console.log("\nTables don't exist yet. You need to run the SQL manually.");
    console.log("The SQL has been saved to: scripts/schema.sql");
    console.log("\nPlease:");
    console.log("1. Open https://supabase.com/dashboard/project/yuauqnfqdcjbnuczpgxf/sql");
    console.log("2. Paste the contents of scripts/schema.sql");
    console.log("3. Click 'Run'");
    console.log("4. Then come back here and I'll verify + continue");
  } else {
    const data = await res.json();
    console.log("style_templates response:", JSON.stringify(data));
  }
}
